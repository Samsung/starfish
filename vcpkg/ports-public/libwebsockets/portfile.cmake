vcpkg_from_github(
    ...
    REF "v${VERSION}"
    SHA512 03607ac112185a673d1892b05d63cea28a2da9f182e5113ad3f6427dc63eea3b342c31e1eb47ff3ca4e394651710a35cde1ad28a931c70114676484224799f19
    HEAD_REF master
    PATCHES
        msvc-warnings.patch
...
# Force the native Windows SChannel TLS backend explicitly rather than rely
# on upstream's WIN32-only default (CMakeLists.txt: LWS_SCHANNEL_DEFAULT) --
# this port's vcpkg.json dropped the "openssl" dependency on Windows on the
# assumption that SChannel, not OpenSSL, backs TLS there, so if that default
# ever flips upstream this should fail configure loudly (no OpenSSL to find)
# instead of silently linking whatever happens to be on the machine.
if(VCPKG_TARGET_IS_WINDOWS)
    set(LWS_WITH_SCHANNEL_VALUE ON)
else()
    set(LWS_WITH_SCHANNEL_VALUE OFF)
endif()
...
vcpkg_cmake_configure(
    ...
    OPTIONS
        -DLWS_WITH_LIBUV=OFF
        -DLWS_WITH_SCHANNEL=${LWS_WITH_SCHANNEL_VALUE}
        ...
        # HTTP/3 (QUIC transport), on by default whenever a TLS backend that
        # supports it (schannel included) is selected. Starfish only ever
        # speaks plain ws(s):// over the H1/WS roles, which stay on
        # regardless of this; QUIC's role files also aren't C4244-clean
        # under x86-windows' /WX (uint64_t truncating to a 32-bit size_t),
        # so leaving unused code compiling in there just to fight its
