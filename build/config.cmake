...
# STARFISH_ENABLE_WORKER : enable worker
...
ELSEIF (${BACKEND} STREQUAL "glfw_cairo_gl")
    ...
    ENDIF()
ELSEIF (${BACKEND} STREQUAL "x11_cairo_gl")
    IF (${HOST} STREQUAL "linux")
        SET (LWE_DEFINES_BACKEND -DSTARFISH_X11_CAIRO_GL)
    ENDIF()
...
IF (${ENABLE_SERVICE_WORKER} STREQUAL "1")
    ...
    SET (LWE_DEFINES_CUSTOM ${LWE_DEFINES_CUSTOM} ${SERVICE_WORKER_CXXFLAGS})
ELSEIF (${ENABLE_WORKER} STREQUAL "1")
    SET (WORKER_CXXFLAGS
        -DSTARFISH_ENABLE_WORKER
    )
    SET (LWE_DEFINES_CUSTOM ${LWE_DEFINES_CUSTOM} ${WORKER_CXXFLAGS})
ENDIF()
...
SET (LWE_CXXFLAGS ${LWE_CXXFLAGS_DEFAULT} ${LWE_CXXFLAGS_COMPILER} ${LWE_CXXFLAGS_HOST} ${LWE_CXXFLAGS_BACKEND} ${LWE_CXXFLAGS_MODE} ${LWE_CXXFLAGS_LTO} ${LWE_CXXFLAGS_ASAN} ${LWE_CXXFLAGS_COVERAGE} ${CXXFLAGS_FROM_ENV})
...
ELSEIF (${BACKEND} STREQUAL "efl" AND ${HOST} STREQUAL "tizen")
    pkg_check_modules (STARFISH_BACKEND REQUIRED freetype2 fontconfig harfbuzz elementary ecore ecore-imf-evas efl-extension)
    IF (${BUILD_CAIRO} STREQUAL "0")
        pkg_check_modules (STARFISH_BACKEND_CAIRO REQUIRED cairo)
    ENDIF()
...
ELSEIF ((${BACKEND} STREQUAL "glfw_cairo_gl" OR ${BACKEND} STREQUAL "x11_cairo_gl") AND ${ARCH} STREQUAL "x64")
    ...
    IF (${WEBGL} STREQUAL "1")
        pkg_check_modules (STARFISH_BACKEND REQUIRED ${DEFAULT_PACKAGES} egl)
    ELSE()
        pkg_check_modules (STARFISH_BACKEND REQUIRED ${DEFAULT_PACKAGES})
    ENDIF()
...
ELSEIF (${BACKEND} STREQUAL "ecore_wayland2_cairo_gl" AND ${HOST} STREQUAL "tizen")
    pkg_check_modules (STARFISH_BACKEND REQUIRED freetype2 fontconfig harfbuzz elementary ecore ecore-imf ecore-wl2 wayland-client egl gles20 )
...
IF (${HOST} STREQUAL "tizen")
    ...
    ELSEIF (${CUSTOM} STREQUAL "unified_tv")
        pkg_check_modules (STARFISH_TIZEN_CUSTOM REQUIRED dlog capi-network-connection capi-media-player capi-media-audio-io)
    ELSEIF (${CUSTOM} STREQUAL "headless")
        pkg_check_modules (STARFISH_TIZEN_CUSTOM REQUIRED dlog capi-network-connection capi-media-player)
    ELSEIF (${CUSTOM} STREQUAL "prod_tv")
...
IF (${BACKEND} MATCHES "efl_cairo" OR ${BACKEND} STREQUAL "efl_skia_gl" OR ${BACKEND} STREQUAL "efl_skia_gb" OR ${BACKEND} STREQUAL "ecore_wayland2_cairo_gl" OR ${BACKEND} STREQUAL "dali" OR ${BACKEND} STREQUAL "flutter")
    IF (${USE_EMBEDDED_IMAGE_DECODER} STREQUAL "1")
        SET (STARFISH_LIBRARIES_BACKEND png16 z jpeg gif)
    ELSE()
        IF (${HOST} STREQUAL "tizen")
    ...
    ELSEIF (${BACKEND} STREQUAL "dali" AND ${HOST} STREQUAL "tizen" AND ((${TIZEN_MAJOR_VERSION} LESS 5) OR (${TIZEN_MAJOR_VERSION} EQUAL 5)))
        SET (STARFISH_LIBRARIES_BACKEND ${STARFISH_LIBRARIES_BACKEND} dali-core dali-adaptor dali-toolkit)
    ENDIF()
ELSEIF (${BACKEND} STREQUAL "glfw_cairo_gl" OR ${BACKEND} STREQUAL "x11_cairo_gl")
    IF (${BACKEND} STREQUAL "glfw_cairo_gl")
        SET (STARFISH_LIBRARIES_BACKEND GL GLESv2 glfw)
    ELSEIF (${BACKEND} STREQUAL "x11_cairo_gl")
        SET (STARFISH_LIBRARIES_BACKEND GLESv2 EGL X11)
    ENDIF()
    IF (${ARCH} STREQUAL "x64")
        IF (${USE_EMBEDDED_IMAGE_DECODER} STREQUAL "0")
            SET (STARFISH_LIBRARIES_BACKEND ${STARFISH_LIBRARIES_BACKEND} turbojpeg gif jpeg webp)
        ENDIF()
    ENDIF()
ENDIF()
