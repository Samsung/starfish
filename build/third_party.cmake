...
IF (${HOST} STREQUAL "linux" OR ${CUSTOM} STREQUAL "prod_tv" OR ${CUSTOM} STREQUAL "unified_tv" OR ${CUSTOM} STREQUAL "unified_mobile")
    ...
    ELSEIF (${HOST} STREQUAL "linux")
        SET (LIBWEBSOCKETS_BUILD_OPTION -DSTARFISH_CUSTOM=1 -DLWS_MAX_SMP=1 -DLWS_CLIENT_HTTP_PROXYING:BOOL=OFF -DLWS_HAVE_VISIBILITY:BOOL=ON -DLWS_STATIC_PIC:BOOL=OFF
            ...
            -DOPENSSL_ROOT_DIR=${CMAKE_SYSROOT}/usr
            -DLWS_OPENSSL_INCLUDE_DIRS=${CMAKE_SYSROOT}/usr/include)
        ADD_CUSTOM_COMMAND (OUTPUT ${LIBWEBSOCKETS_LOCAL_TARGET}
                            DEPENDS ${LIBWEBSOCKETS_BUILD_DIR}/libwebsocket_copied
                            WORKING_DIRECTORY ${LIBWEBSOCKETS_BUILD_DIR}
                            COMMENT "BUILD LIBWEBSOCKETS"
                            COMMAND ${CMAKE_COMMAND} -E make_directory ${LIBWEBSOCKETS_BUILD_OUTDIR}
                            COMMAND test -f ${LIBWEBSOCKETS_LOCAL_TARGET} || ${CMAKE_COMMAND} -S . -B${LIBWEBSOCKETS_BUILD_OUTDIR} -G Ninja ${LIBWEBSOCKETS_BUILD_OPTION} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER}
                            COMMAND test -f ${LIBWEBSOCKETS_LOCAL_TARGET} || ${CMAKE_COMMAND} --build ${LIBWEBSOCKETS_BUILD_OUTDIR}
                            COMMAND touch ${LIBWEBSOCKETS_LOCAL_TARGET}
        )
    ELSE()
        ...
        ADD_CUSTOM_COMMAND (OUTPUT ${LIBWEBSOCKETS_LOCAL_TARGET}
                            ...
                            COMMENT "BUILD LIBWEBSOCKETS"
                            COMMAND ${CMAKE_COMMAND} -E make_directory ${LIBWEBSOCKETS_BUILD_OUTDIR}
                            COMMAND test -f ${LIBWEBSOCKETS_LOCAL_TARGET} || ${CMAKE_COMMAND} -S . -B${LIBWEBSOCKETS_BUILD_OUTDIR} -G Ninja ${LIBWEBSOCKETS_BUILD_OPTION} -DCMAKE_C_COMPILER=${THIRD_PARTY_C_COMPILER_OPTION} -DCMAKE_CXX_COMPILER=${THIRD_PARTY_CXX_COMPILER_OPTION}
                            COMMAND test -f ${LIBWEBSOCKETS_LOCAL_TARGET} || ${CMAKE_COMMAND} --build ${LIBWEBSOCKETS_BUILD_OUTDIR}
                            COMMAND touch ${LIBWEBSOCKETS_LOCAL_TARGET}
        )
    ENDIF()
...
IF (${HOST} STREQUAL "linux")
    ...
    IF (CMAKE_CROSSCOMPILING AND ${ARCH} STREQUAL "aarch64")
        SET (OPENSSL_CONFIGURE_CMD ./Configure linux-aarch64 --cross-compile-prefix=aarch64-linux-gnu- --prefix=${OPENSSL_BUILD_PATH})
    ELSE()
