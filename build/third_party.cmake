...
IF (${ARCH} STREQUAL "tizen")
    ...
    ADD_CUSTOM_TARGET (library_copy
        DEPENDS ${GCUTIL_ROOT}/bdwgc/out/tizen_obs/arm/${MODE}.shared/.libs/libgc.so  ${ESCARGOT_ROOT}/libescargot.a ${THIRD_PARTY_ROOT}/libtuv/build/noarch-tizen/debug/lib/libtuv.a
        #COMMAND ${CMAKE_COMMAND} -E copy ${STARFISH_ROOT}/tizen_dep/arm/libzmq.a ${OUTPUT_DIRECTORY}/lib/tizen
        COMMAND ${CMAKE_COMMAND} -E copy ${GCUTIL_ROOT}/bdwgc/out/tizen_obs/arm/${MODE}.shared/.libs/libgc.so ${OUTPUT_DIRECTORY}/lib
        COMMAND ${CMAKE_COMMAND} -E copy ${ESCARGOT_ROOT}/libescargot.a ${OUTPUT_DIRECTORY}/lib
        COMMAND ${CMAKE_COMMAND} -E copy ${THIRD_PARTY_ROOT}/libtuv/build/noarch-tizen/debug/lib/libtuv.a ${OUTPUT_DIRECTORY}/lib
    )
    ADD_LIBRARY (tuv STATIC STATIC IMPORTED)
    ADD_DEPENDENCIES (tuv library_copy)
    SET_PROPERTY (TARGET tuv PROPERTY IMPORTED_LOCATION ${OUTPUT_DIRECTORY}/lib/libtuv.a)
ELSE()
    ADD_CUSTOM_TARGET (build_third_party
        COMMAND ./build_third_party.sh x86
    )
    ADD_CUSTOM_TARGET (library_copy
        DEPENDS ${THIRD_PARTY_ROOT}/zeromq/out/${HOST}/${ARCH}/${MODE}.shared/.libs/libzmq.so ${GCUTIL_ROOT}/bdwgc/out/${HOST}/${ARCH}/${MODE}.shared/.libs/libgc.so ${ESCARGOT_ROOT}/out/${HOST}/${ARCH}/interpreter/${MODE}/libescargot.a
...
SET_PROPERTY (TARGET gc PROPERTY IMPORTED_LOCATION ${OUTPUT_DIRECTORY}/lib/libgc.so)
ADD_DEPENDENCIES (gc library_copy)

ADD_CUSTOM_TARGET (prepare_build
    DEPENDS build_third_party generate_binding
)
