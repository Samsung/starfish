...
IF (${USE_LIBWEBSOCKETS} STREQUAL "1")
    ...
    ADD_CUSTOM_COMMAND (OUTPUT ${LIBWEBSOCKETS_BUILD_DIR}/libwebsocket_copied
                        ...
                        # lws_config.h is generated by the sub-build into its own
                        # binary dir, but the source include dir comes first on the
                        # include path. A leftover lws_config.h in the source tree
                        # (in-source lws build, or one carried along by a release
                        # snapshot) therefore shadows the generated one, and the
                        # feature set the code is compiled against stops matching
                        # the one cmake used to pick the sources. With
                        # CMAKE_SYSTEM_NAME=Tizen lws turns LWS_WITH_NETLINK and
                        # LWS_WITH_CACHE_NSCOOKIEJAR off, so a stale header that
                        # still has them on leaves libwebsockets_lwe.so with
                        # undefined role_ops_netlink / _lws_route_est_outgoing /
                        # lws_cache_ops_nscookiejar / lws_parse_set_cookie /
                        # lws_cookie_send_cookies. Drop it so the generated one wins.
...
IF (CMAKE_SYSTEM_NAME STREQUAL "Linux")
    ...
    IF (CMAKE_CROSSCOMPILING)
        ...
        SET (OPENSSL_CONFIGURE_CMD env "CC=${CMAKE_C_COMPILER}" "CXX=${CMAKE_CXX_COMPILER}" "CFLAGS=$ENV{CFLAGS}" "CXXFLAGS=$ENV{CXXFLAGS}" "LDFLAGS=$ENV{LDFLAGS}" ./Configure ${OPENSSL_CONFIGURE_TARGET} --prefix=${OPENSSL_BUILD_PATH} ${OPENSSL_SYSROOT_OPTION} no-asm shared no-fips)
    ELSE()
