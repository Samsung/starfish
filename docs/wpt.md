...
`wpt_serve_reftest`/`wpt_serve_crashtest` run un-curated lists straight from
MANIFEST.json (see below), so unlike `wpt_serve_all` they are not expected to
be ~100% yet — baseline and annotate them like any other list (see
`tool/CLAUDE.md`'s WPT workflow) before relying on them as a CI gate.
...
corpus to carry forward). They are **un-curated**: a fresh run shows real
failures, not ~100%, until baselined and annotated the same way as the
testharness lists (`tool/wpt_runner.py --mode ... --results` +
`wpt_annotate.py`) — see the WPT workflow in `tool/CLAUDE.md`.
