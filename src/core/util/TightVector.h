...
#ifndef __StarfishTightVector__
#define __StarfishTightVector__
...
public:
    ...
    TightVector(_InputIterator start, _InputIterator end)
        : m_buffer(nullptr)
        , m_size(0)
    {
        construct(start, end);
    ...
    TightVector<T, Allocator>& operator=(const TightVector<T, Allocator>& other)
    {
        if (other.m_size > 0) {
            deallocate();
            construct(other);
...
protected:
    ...
    VectorAllocInfo<T> allocate(size_t size) const
    {
        ...
        return VectorAllocInfo<T>(ret, size);
    }
    ...
    void construct(const TightVector<T, Allocator>& other)
    {
