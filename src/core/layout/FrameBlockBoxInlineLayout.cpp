...
void* LineBox::operator new(size_t size)
{
    ...
    if (!typeInited) {
        GC_word desc[GC_BITMAP_SIZE(LineBox)] = { 0 };
        LineBox::fillGCDescriptor(desc);
        descr = GC_make_descriptor(desc, GC_WORD_LEN(LineBox));
        typeInited = true;
    }
...
LineFormattingContext::LineFormattingContext(FrameBlockBox* block,
    ...
    , m_block(block)
    , m_layoutContext(ctx)
    , m_currentLayoutParent(nullptr)
    , m_isPendingBreakLine(false)
    , m_isWhiteSpaceAtLast(false)
    , m_isSoftHyphenAtLast(false)
    , m_canConcatWord(false)
    , m_isLastLineBox(false)
    , m_isFirstLineCandidate(false)
    , m_shouldConsiderTextOverflow(false)
    , m_inlineBoxIndex(0)
    , m_pendingFloatingBoxNumsBeforeCurrentLine(0)
    , m_floatingBoxesSizeBeforeCurrentLine(0)
...
void* InlineNonReplacedBox::operator new(size_t size)
{
    ...
    static GC_descr descr;
    if (!typeInited) {
        GC_word desc[GC_BITMAP_SIZE(InlineNonReplacedBox)] = { 0 };
        InlineNonReplacedBox::fillGCDescriptor(desc);
        descr = GC_make_descriptor(desc, GC_WORD_LEN(InlineNonReplacedBox));
        typeInited = true;
    }
    return GC_MALLOC_EXPLICITLY_TYPED(size, descr);
}
...
LayoutUnit FrameBlockBox::layoutInline(LayoutContext& ctx)
{
...

    {
        ...
        if (needsTestingTextOverflow) {
            ...
            LayoutUnit rightBoundary = width() -
...
void FrameFlexibleBox::computePreferredWidth(PreferredWidthContext& ctx)
{
    ...
    if (width.isDefinite(false)) {
        ...
        if (fCtx.isMainAxisInInlineAxis()) {
            ...
            f = firstChild();
            LayoutUnit w;
            LayoutUnit totalPreferredMinWidth;
            LayoutUnit maxPreferredMinWidth;
            while (f) {
                if (!f->isFlexItem()) {
                    f = f->next();
                    continue;
                ...
                auto preferredWidths =
                    ctx.preferredWidthsWithNewContext(f->asFrameBox());
                totalPreferredMinWidth += preferredWidths.second;
                if (maxPreferredMinWidth < preferredWidths.second) {
                    maxPreferredMinWidth = preferredWidths.second;
                }
                f = f->next();
            }
            ...
            if (w > ctx.remainingWidth() && !ctx.isIntrinsicMode()) {
                ...
                LayoutUnit minWidth;
                if (maxPreferredMinWidth > ctx.remainingWidth()) {
                    minWidth = totalPreferredMinWidth;
                } else {
                    minWidth =
                        std::min(totalPreferredMinWidth, maxPreferredMinWidth);
                }
                ...
                if (flexWrap == NoWrapFlexWrapValue) {
                    ...
                    if (maxPreferredMinWidth > ctx.remainingWidth()) {
                        ctx.updatePreferredMinWidth(maxPreferredMinWidth);
                        ctx.updatePreferredWidth(maxPreferredMinWidth);
                    } else {
                ...
                ctx.updatePreferredWidth(w);
                ctx.updatePreferredMinWidth(totalPreferredMinWidth);
            }
        } else {
            Frame* f = firstChild();
            LayoutUnit w;
