                             ...
                             DirectionValue direction)
{
    ...
    if (direction == LtrDirectionValue) {
        ...
        child->setX(parent->width() - child->width() - mX -
                    parent->borderRight() - parent->paddingRight());
    }
}
...
LayoutUnit FrameBlockBox::layoutBlock(LayoutContext& ctx)
{
    ...
    reLayoutFrameBox:
        ...
        LayoutUnit clearedDistanceToFloatBottom =
            ctx.clearedDistanceToFloatBottom(
                yAbsPosition, child->style()->clear(), &floatIdx);

        if (clearedDistanceToFloatBottom > 0) {
            // TODO : Study more cases which can set clearAffected to true.
            if (clearedDistanceToFloatBottom >
        ...
        // Basically, isSelfCollapsing isn't changed. But for the case that this
        // margin collapse of child box is ignored, and the margin parts
        // is included to height, even though it was originally self collapsing,
        // but it shouldn't behave like that.
        ...
        if (shouldAvoidFloatingBox(child)) {
            ...
            LayoutLocation selfLoc =
                child->asFrameBox()->absolutePoint(ctx.frameDocument());
            LayoutUnit originalY = selfLoc.y();
        rePositionFloatAvoidingFrameBox:
            std::pair<LayoutUnit, LayoutUnit> boundaries =
                ctx.horizontalBoundaryBetweenFloatingBoxes(
                    selfLoc.y(), child->asFrameBox()->height(), leftBoundary,
                    rightBoundary);
            if ((boundaries.first != leftBoundary &&
                 selfLoc.x() < boundaries.first) ||
                (boundaries.second != rightBoundary &&
                 selfLoc.x() + child->asFrameBox()->width() >
                     boundaries.second)) {
                floatAffected |= true;
            }
            ...
            if (floatAffected) {
                ...
                if ((hasToStretchWidth &&
                    ...
                    if (hasToStretchWidth) {
                        child->asFrameBox()->moveX(boundaries.first -
                                                   selfLoc.x());
                        reLayoutNeeded = width != child->asFrameBox()->width();
                        child->asFrameBox()->setContentWidth(
                            child->asFrameBox()->contentWidth() + width -
                            child->asFrameBox()->width());
                    } else {
                        if (boundaries.first != leftBoundary &&
                        ...
                        if (boundaries.second != rightBoundary &&
                            selfLoc.x() + child->asFrameBox()->width() >
                                boundaries.second) {
                            child->asFrameBox()->moveX(
                                boundaries.second - selfLoc.x() -
                                child->asFrameBox()->width());
                        }
                    }
                    ...
                    if (reLayoutNeeded) {
                        ...
                        // The only possible case there has floating box is the
                        // child box is FrameBlockBox, and this box
                        // established BlockFormattingContext. This means when
                        // the layout is finished, the floating box cached
                        // vector
        ...
        if (isSelfCollapsing) {
            ...
            // height = max(max(max(SCB1.maxMargin, SCB2.maxMargin, ...,
            // SCBn.maxMargin), SCC.matginTop), 0)
            //
            if (clearAffected) {
                yPositionOfClearedSelfCollapsingBlock =
                    child->asFrameBox()->y();
                marginTopOfClearedSelfCollapsingBlock =
                    child->asFrameBox()->marginTop();
                maxMarginOfSelfCollapsingBlocks =
                    child->asFrameBox()->marginBottom();
                ...
                maxMarginOfSelfCollapsingBlocks =
                    std::max(maxMarginOfSelfCollapsingBlocks,
                             child->asFrameBox()->marginTop());
                maxMarginOfSelfCollapsingBlocks =
                    std::max(maxMarginOfSelfCollapsingBlocks,
                             child->asFrameBox()->marginBottom());
            ...
            if (wasClearedSelfCollapsingBlock) {
                LayoutUnit height = maxMarginOfSelfCollapsingBlocks -
                                    marginTopOfClearedSelfCollapsingBlock;
                if (height < 0) {
                    height = 0;
                }
                if (maxNormalFlowBottom <
                    height + yPositionOfClearedSelfCollapsingBlock) {
                    maxNormalFlowBottom =
                        height + yPositionOfClearedSelfCollapsingBlock;
                }
                normalFlowHeight =
                    height + yPositionOfClearedSelfCollapsingBlock - top;
            }
        } else {
            wasClearedSelfCollapsingBlock = false;
            if (maxNormalFlowBottom <
                child->asFrameBox()->height() + child->asFrameBox()->y()) {
                maxNormalFlowBottom =
                    child->asFrameBox()->height() + child->asFrameBox()->y();
            }
            normalFlowHeight =
