...
#ifndef __StarFishEventSourceResourceRequestJobDelegate__
#define __StarFishEventSourceResourceRequestJobDelegate__
...
class EventSourceWorkerHelper;
class HTTPHeaderMap;
class HTTPTransaction;

struct EventSourceWorkerData {
    bool isAborted;
    bool isRedirected;
    long lastTransactionResponseCode;
    ResourceRequest* request;
    EventSourceWorkerHelper* eventSourceNetworkWorker;
    std::unique_ptr<HTTPTransaction> httpTransaction;
    std::string lastLocation;
};

class EventSourceWorkerHelper : public gc {
public:
    EventSourceWorkerHelper()
    {
    }
    virtual ~EventSourceWorkerHelper()
    {
    }
...
class AsyncEventSourceWorkHelper : public EventSourceWorkerHelper {
protected:
    virtual void responseHandlerWrapper(int res,
                                        EventSourceWorkerData* requestData);
};

class SyncEventSourceWorkHelper : public EventSourceWorkerHelper {
protected:
    virtual void responseHandlerWrapper(int res,
                                        EventSourceWorkerData* requestData);
};

class EventSourceResourceRequestJobDelegate
    : public gc,
...
public:
    ...
    static size_t curlWriteHeaderCallback(void* ptr, size_t size, size_t nmemb,
                                          void* data);
    EventSourceResourceRequestJobDelegate(ResourceRequest* proxy);
