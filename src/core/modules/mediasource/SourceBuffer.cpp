...
#define SOURCEBUFFER_TYPE(sb)                 \
    (sb->type()->charAt(0) == 'v'   ? "video" \
     : sb->type()->charAt(0) == 'a' ? "audio" \
                                    : "etc")
...
void SourceBuffer::setUpdating(bool flag, UpdateState state)
{
    ...
    for (size_t i = 0; i < events.size(); i++) {
        m_parentMediaSource->attachedMediaElement()->addEventToOperationQueue(
            this, new Event(executionContext(), events[i]));
    }
...
bool SourceBuffer::codedFrameEviction(size_t newDataSize)
{
    ...
    if (maxAssume >= m_parentMediaSource->availableBufferSize() / 4) {
        ...
        uint64_t targetPos = (uint64_t)(playbackPos * 1000);
...
TimeRanges* SourceBuffer::buffered()
{
    ...
    //    ranges.
    m_buffered = new TimeRanges(document()->executionContext());
