...
const requires = (i, args) => {
  if (args.length < i) {
    throw new TypeError(
      `${i} argument required, but only ${args.length} present.`,
    );
  }
};
...
function openDB() {
  return new Promise((rs) => {
    ...
    open.onupgradeneeded = () => {
      ...
      const cacheStore = db.createObjectStore('caches', {
        autoIncrement: true,
      });
...
*/
  ...
  async put(req, res) {
    ...
    if (!/^((http|https):\/\/)/.test(req.url)) {
      throw new TypeError(
        `Request scheme '${req.url.split(':')[0]}' is unsupported`,
      );
    }
  ...
  async delete(request, options = {}) {
    ...
    query.onsuccess = function () {
      ...
      if (cursor) {
        if (
          url === cursor.value.reqUrl &&
          (ignoreMethod || method === cursor.value.reqMethod)
        ) {
          deleted = true;
    ...
    return new Promise((rs) => (tx.oncomplete = () => rs(deleted)));
  }
