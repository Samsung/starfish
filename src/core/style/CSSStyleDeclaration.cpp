...
//  https://github.com/lattera/freebsd/blob/master/lib/libc/string/strnstr.c#L42
/*
    ...
    bool isImportant)
{
    ...
    if (UNLIKELY(keyKind == CSSStyleValuePair::KeyKind::Content)) {
        ...
        bool hasCustomProperty = strnstr(value, "--", valueLength);
        tokenizeCSSValue(tokens, value, valueLength, ",", 1, hasCustomProperty);
    }
                                       ...
                                       bool isImportant)
{
    ...
    for (size_t i = 0; i < layerSize; i++) {
        ...
        tokenizeCSSValue(tokens, layers[i].data(), layers[i].length(), "", 0,
                         true);
        if (v0.updateValueVarReferences(tokens)) {
            v0.setValue(String::fromUTF8(value, length));
            v0.setFlagImportant(isImportant);
            addCSSValuePair(CSSStyleValuePair::KeyKind::Animation, v0);
            return;
        } else if (layerSize == 1 && v0.updateValueCommon(tokens)) {
