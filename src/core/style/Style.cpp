                       ...
                       bool (&damagedKeys)[CSSStyleValuePair::KeyKindSize])
{
    ...
    if (oldStyle && oldStyle->display() != DisplayValue::NoneDisplayValue &&
        ...
        if (applyTransitionIfNeeds(element, oldStyle, oldFrame, style,
                                   damagedKeys, canceledAnimationProgress)) {
            elementHasAnimation = true;
            needsToCheckActiveAnimationExecutorInWebView = true;
        }
    }
            ...
            if (activeAnimations[i]->targetElement() == element &&
                ...
                if (shouldRemove == false) {
                    ...
                    if (data == nullptr) {
                        ...
                        for (size_t j = 0; j < data->keyframesSize(); j++) {
                            auto keyframes = data->keyframes(j);
                                           ...
                                           const CSSTokenVector& tokens)
{
    ...
    if (parts.size() == 1) {
        ...
        if (result) {
            ...
            multiValue()->emplace_back(rs.valueKind(), rs.value());
            re.setValueKind(CSSStyleValuePair::ValueKind::StringValueKind);
            re.setStringValue(String::createASCIIString(parts[1].c_str()));
            multiValue()->emplace_back(re.valueKind(), re.value());
        }
    }
                                              ...
                                              const CSSTokenVector& tokens)
{
    ...
    if (parts.size() == 1) {
        ...
        if (result) {
            ...
            multiValue()->emplace_back(cs.valueKind(), cs.value());
            ce.setValueKind(CSSStyleValuePair::ValueKind::StringValueKind);
            ce.setStringValue(String::createASCIIString(parts[1].c_str()));
            multiValue()->emplace_back(ce.valueKind(), ce.value());
        }
    }
