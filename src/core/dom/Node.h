...
public:
    ...
    // A <slot> in a shadow tree is expanded into its flattened slottables by
    // WHATWG DOM "find flattened slottables", so it is never itself one of the
    // nodes ::slotted() represents. (It does stay in the rendering tree as a
    // boxless node -- see RenderingSiblingIterator.) A <slot> whose root is
    // not a shadow root takes no part in slot assignment and is an ordinary
    // element: selector-matchable like any other, and appended as-is when it
    // is itself slotted into a shadow slot.
