...
void ResourceURL::parseURLString(String* baseURL, String* url)
{
    ...
    if (isBaseUrlValid && !isAbsolute && !baseURL->equals("about:blank")) {
        ...
        if (baseEndsWithSlash || urlLength == 0) {
            url = baseURL->concat(url);
        } else if (url->startsWith("?")) {
            if (!url->substring(0, url->lastIndexOf('?'))->contains(":")) {
                url = baseURL->concat(url);
            }
        } else if (url->startsWith("#")) {
            url = baseURL->concat(url);
...
ReferrerURL::ReferrerURL(String* referrer, ReferrerPolicy policy)
    : ResourceURL(referrer)
    , m_policy(policy)
{
}
...
ReferrerURL::ReferrerURL(ResourceURL* referrer, ReferrerPolicy policy)
    : ResourceURL(*referrer)
    , m_policy(policy)
{
    STARFISH_ASSERT(referrer != nullptr);
