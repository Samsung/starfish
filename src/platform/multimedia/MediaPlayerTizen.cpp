...
static bool isFramerateChanged(const Framerate& before, const Framerate& after)
{
    ...
    if (before.toDouble() > 0 &&
        ...
        for (detectIndex = 0; detectIndex < 12; ++detectIndex) {
            ...
            if (newFrameDetectIndex != 0 && previousFrameDetectIndex != 0)
                break;
        }
...
{
    if (m_inPrepare == true) {
        PLAYER_LOGI("MediaPlayerTizen::seek -> seeking failed saving time %lf",
                    time);
...
{
    PLAYER_LOGI("MediaPlayerTizen::seekOperation() (time: %d)", timeInMS);
...
void MediaPlayerTizen::handleEnded()
{
    if (isMainThread() == true) {
        ...
        if (m_nativePlayer == nullptr ||
            playbackState() == MediaPlayer::PLAYBACK_STATE_END) {
            return;
        }
        if (isMSE() == false && loop() == true) {
            return;
        }
        pause();
        ...
        if (isMSE() == true && m_container->isHTMLVideoElement() == true &&
            m_container->frame()) {
...
void MediaPlayerTizen::prepare(ResourceURL* url)
{
    ...
    ret = player_set_buffering_cb(
        ...
        [](int percent, void* data) {
            PLAYER_LOGI("MediaPlayerTizen -> buffering state... %d", percent);
        },
        this);
    ...
    if (ret != PLAYER_ERROR_NONE) {
        PLAYER_LOGE("MediaPlayerTizen::player_prepare_async return error !!!");
        printNativePlayerError(ret);
        m_foundError = true;
        handlePrepared();
    }
...
void MediaPlayerTizen::handlePrepared()
{
    ...
    PLAYER_LOGI("MediaPlayerTizen::prepare ok %s %s %d %d", videoCodec,
    ...
    if (isMSE() == false) {
        ...
        int ret = playerSetPlayPosition(defaultTimeInMS);
        if (ret != PLAYER_ERROR_NONE) {
            PLAYER_LOGI(
                "MediaPlayerTizen::handlePrepared failed to set default start "
                "pos");
            printNativePlayerError(ret);
            m_foundError = true;
            m_container->giveupFetchingResource();
...
{
    PLAYER_LOGI("MediaPlayerTizen::dispose (%p)", this);
    if (m_nativePlayer != nullptr) {
        pause();
...
{
    PLAYER_LOGI("MediaPlayerTizen::setVolume(%f)", volume);
    if (m_nativePlayer == nullptr) {
        return;
    }
    player_state_e state;
...
{
    PLAYER_LOGI("MediaPlayerTizen::setMuted(%s)", muted ? "true" : "false");
    if (m_nativePlayer == nullptr) {
        return;
    }
    player_state_e state;
                                    ...
                                    const LayoutRect& absVideoRect)
{
    ...
    if (isMSE() == true && playbackState() == MediaPlayer::PLAYBACK_STATE_END) {
        return;
    }
...
void MediaPlayerTizen::fillBuffer(MediaPlayerSourceStream* stream)
{
    Locker<Mutex> locker(*m_fillBufferMutex);
...
void MediaPlayerTizen::enterUnderrunState()
{
    if (isMainThread() == false) {
        ...
        int current = 0;
        player_get_play_position(m_nativePlayer, &current);
        ...
        // NOTE It appears that a fHub device does not invoke
        //      buffer state callback when it is not playing.
        //      Enable belows after several tests on various devices
        // if (playbackState() == PLAYBACK_STATE_PLAYING) {
        //     pause();
        //     m_container->play();
        // }
...
#define RETURN_WHEN_MEDIA_PACKET_ERROR(...)               \
    ...
    while (submitBytes < sizeUpTo) {
        ...
        if (packet.first == nullptr) {
            ...
            DEBUG_STREAMBUFFER_LOG("fillBuffer waiting demuxer[1]");
...
void MediaPlayerTizen::initVideoStreamInfo(size_t initSegmentIndex)
{
    ...
    RETURN_WHEN_PLAYER_ERROR("ERROR: player_set_media_stream_buffer_max_size");
...
void MediaPlayerTizen::initAudioStreamInfo(size_t initSegmentIndex)
{
    ...
    PLAYER_LOGI("> sample_rate : %d", (int)info->audioSampleRate());
    PLAYER_LOGI("> extradata : %d", (int)info->m_extraData.size());
    PLAYER_LOGI("---------------------------------------");
...
void MediaPlayerTizen::updateStreamInfo(MediaPlayerSourceStream* stream,
...
void MediaPlayerTizen::updateVideoStreamInfo(MediaPlayerSourceStream* stream,
                                             size_t pastInitIndex,
                                             size_t newInitIndex)
{
    ...
    if (newInfo->videoHasFramerate() == true &&
        newInfo->videoFramerate().isValid() == true) {
        if (newInfo->videoFramerate().toDouble() > 60.0) {
            newInfo->setVideoFramerate(Framerate(60, 1));
        }
        if (pastInfo->videoHasFramerate() == false ||
            pastInfo->videoFramerate().isValid() == false ||
            isFramerateChanged(pastInfo->videoFramerate(),
                               newInfo->videoFramerate())) {
            hasFramerateChanged = true;
        }
    }
    if (hasFramerateChanged == true) {
        videoFramerateChanged(stream, newInfo->videoFramerate().m_num,
    ...
    PLAYER_LOGI("> index     : %d", (int)newInitIndex);
    PLAYER_LOGI("> size      : %lu x %lu", m_videoWidth, m_videoHeight);
    PLAYER_LOGI("> max_buffer: %llu",
                (long long unsigned int)stream->maxBufferSize());
    PLAYER_LOGI("> avg_frame_rate: %d/%d(%f)", newInfo->videoFramerate().m_num,
                newInfo->videoFramerate().m_den,
                newInfo->videoFramerate().toDouble());
    PLAYER_LOGI("---------------------------------------");
}
